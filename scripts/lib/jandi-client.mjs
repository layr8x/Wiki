...
//   2) "OOO 학생/학부모/자녀/보호자" 문맥의 이름을 가림(이름→라벨 순서)
// ⚠️ 실사용 패턴 재검토(2026-07): 기존엔 "학생 OOO"(라벨→이름) 순서만 가렸는데, 라이브 데이터
// 실측 결과 이 방향은 정밀도가 사실상 0에 가까웠다(라벨 뒤 아무 일반 명사나 "이름"으로 오인해
// "학생 대상"→"학생 ***", "학생 여러분"→"학생 ***" 처럼 애먼 단어만 지우고 실제 이름은 못 잡음
// — 잔디 대화 2,952건이 이미 이렇게 훼손됨). 반면 실제 화면에 노출되던 진짜 사례
...
export function maskCustomerInfo(text) {
  ...
  s = s.replace(STUDENT_NAME_CTX_RE, (m, name, paren, label) =>
