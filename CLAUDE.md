...
- **측정기 = `tools/writing/ko_ai_score.py`** (저장소 내장, 의존성 없음): `python3 tools/writing/ko_ai_score.py 글.txt --mj` (--mj = 명준 실측 기준). 10개 지표(쉼표 문장비율·문장 CV·상투어·번역투·문두 접속부사·줄표·첫째둘째·종결 쏠림·"것" 밀도·문단 균일성), 근거 태그([논문]=KatFishNet ACL 2025 / [실측] / [카탈로그] / [휴리스틱]) 표기. AI 표본 위험5 vs 명준 표본 위험0 분리 검증됨. 정밀판 `ko_ai_score_kiwi.py`(kiwipiepy 형태소 분석, 병행 세션 제작)도 병존.
- **교정 원칙 4개(가이드 4장 A-2, 병행 세션 실전 검증)**: ①상투어 0이어도 반복(같은 표현·템플릿)이 문서 단위 AI 티를 만든다 ②습니다체 종결 쏠림·데이터 보고서 명사 비율은 장르 특성이라 고치지 않는다 ③쉼표는 삭제가 아니라 문장 분할로 줄인다(굿하트 함정 금지) ④사람 손 닿은 문서는 최소 개입 정밀 수술, 순수 AI 초안만 전면 재조립. 병행 세션 연구 원본 = `analysis/한국어_휴머나이징_연구정리_20260714.md`.
