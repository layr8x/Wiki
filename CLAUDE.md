...
10. **★ 작업 하나가 끝날 때마다 진행 리포트를 붙일 것** (2026-08-20 사용자 지시, 예외 없음)
    - 내가 요청한 **전체 작업 흐름** 안에서 **어디를 / 어떻게 / 얼마나** 했는지 매번 보고한다.
    - **진행률 %**와 **완성도 10점 만점 점수**를 반드시 숫자로 표기한다. 점수는 근거와 함께.
    - **완성도가 9.9 미만이면 그 작업을 끝내지 말 것.** 9.9에 못 미치면 "완료"라고 보고하지 말고,
      무엇이 모자란지 적고 9.9 이상이 될 때까지 계속 작업한다. (0장 "검증 전 완료 금지"의 수치화 버전.)
    - 점수를 매길 땐 **검증한 것과 검증 못 한 것을 구분**한다. 안 돌려본 코드는 점수를 깎는다.

11. **★ 짧게, 쉽게, 아껴서** (2026-08-20 사용자 지시)
    - **모든 보고는 간략하게 핵심만.** 표 1~2개 + 짧은 문장. 과정 나열·중복 설명 금지.
    - **전문용어를 꼭 써야 하면 초등학생도 알 수 있게 풀어쓴다.** 비유 1줄이면 충분.
    - **토큰을 아낀다**: 파일은 필요한 줄만 읽고, 도구 호출은 묶어서 한 번에,
      같은 것을 다시 읽지 않는다. 검증은 유지하되 출력은 필요한 줄만 뽑아 본다.
