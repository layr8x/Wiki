...
-- 매일 08:50 KST(=23:50 UTC) 유지보수 cron: (1) 신규 유입 중 '기타'로 떨어진 대화를 rule_v3 로 재분류,
--   (2) 감정 정제사전(lexicon_v2)으로 neutral -> negative 재보정. 둘 다 무비용 SQL이라 분류 함수
--   재배포 없이 기타 비중·감정 정확도를 유지한다. (함수 RULE_ENGINE / 감정 사전과 동일 규칙, 함께 갱신.)
select cron.schedule('kakao-reclassify-etc', '50 23 * * *', $cmd$
  with tgt as (
    select c.chat_id,
      coalesce((select string_agg(message,' ' order by sent_at asc) from (
        ...
        where chat_id=c.chat_id and sender_type='user' and message is not null order by sent_at asc limit 3) q), c.last_message,'') as t
  ...
  update kakao_partner_chats c set category=res.cat, category_confidence=0.65, category_model='rule_v3', category_classified_at=now()
  from res where res.chat_id=c.chat_id and res.cat <> '기타';

  update kakao_partner_messages set sentiment='negative', sentiment_score=-0.4, sentiment_model='lexicon_v2', sentiment_classified_at=now()
