...
//
// 왜 필요한가: 잔디 access token(수명 ~12h)이 만료되고 맥 스튜디오 갱신이 실패하면 5개 방
    ...
    const msg =
      remain <= 0
        ? '🔴 *잔디 수집 토큰이 만료됐어요*\n' +
          ...
          '👉 잔디 토큰을 다시 발급해 주세요(맥 스튜디오 전용 크롬 프로필 재로그인). 넣으면 자동으로 다시 수집돼요.'
        : '🟠 *잔디 수집 토큰이 곧 만료돼요*\n' +
          '조금 뒤 만료되는데 아직 새 토큰이 안 들어왔어요. 지금 자동 갱신이 안 되고 있을 수 있어요.\n' +
          '👉 맥 스튜디오 잔디 토큰 자동 갱신이 도는지 확인해 주세요. 안 되면 전용 크롬 프로필을 한 번 재로그인해 주세요.';
    await sendSlack(msg);
