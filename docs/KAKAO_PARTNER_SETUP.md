...
## ★★ 현재 구조 (2026-08-13 개편) — 수집은 클라우드, 기기는 쿠키 공급만
...
쿠키가 갱신 없이 얼마나 버티는지는 **미측정**이다. 알럿봇이 만료를 잡아 알린다.

### ★ "클라우드 IP 차단" 결론은 틀렸다 — 재측정으로 확정 (2026-08-13)

2026-08-12 의 실측(맥 200 / 클라우드 401)은 사실이지만 **해석이 틀렸다.** 두 호출 사이 4분을
변수로 세지 않았다 — 그 사이 기기 Chrome 이 세션 토큰을 굴려 보관함 스냅샷이 이미 무효였다.
...
이어서 `kakao-collect` Edge Function 을 실제 호출해 **LIVE 채널 101건 적재까지 확인**했다.
→ **클라우드 수집이 가능하다.** 막힌 것은 IP 가 아니라 쿠키 신선도였다.

```bash
node --env-file=.env.local scripts/kakao-cloud-ip-test.mjs
```

**아래 "무슨 일이 있었나 / 진짜 원인" 절의 IP 차단 서술은 이력으로만 읽을 것.**
...
```
Chrome (business.kakao.com 로그인 유지) — 담당자 기기
        │  ① 30분마다: 쿠키 추출 + "실제로 통하는지" 검증 후에만 배달
        ▼
  Supabase 보관함 (kakao_partner_secrets)
        │
        │  ② 5분마다: 클라우드(pg_cron → kakao-collect)가 그 쿠키로 수집
        ▼
...
| 잡 | 어디 | 주기 | 하는 일 |
|---|---|---|---|
| `com.amswiki.kakao-cookie-refresh` | 기기 launchd | 30분 | 쿠키 추출·검증·배달 |
| pg_cron jobid 1 | 클라우드 | 5분 | **수집 본체** |
...
노트북이라 뚜껑을 닫으면 **쿠키 갱신**이 멈춘다(수집은 클라우드라 계속 돈다).
상시 갱신하려면 전원 연결 후 `sudo pmset -c sleep 0 disablesleep 1`.

Supabase 수집 크론(jobid 1)은 2026-08-13 에 **다시 켰다**(5분).
끄려면: `select cron.alter_job(1, active := false);`
...
`[측정]` 2026-08-13 실행 결과 **그 646개는 카카오 서버에도 로그가 없는 방**으로 확인됐고
(`kakao_backfill_empty` 646건 = 현재 빈 방 646건, 완전 일치) 복구할 것이 없다. 이 절은
앞으로 같은 사고가 났을 때의 절차로 남긴다.
