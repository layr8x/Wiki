...
**둘. 영구 지침 라우팅은 공급사 문구 대신 우리가 다시 써서 넣었습니다.**
...
설치 시점에는 넣지 않았고, **같은 날 사용자가 "자동으로 켜줘"라고 지시해서 켰습니다.**
다만 공급사가 준 영문 블록을 그대로 붙이지 않고 세 가지를 바꿔 다시 썼습니다.

1. **범위를 좁혔습니다.** 원문은 "모든 제품 UI 작업"이라 백엔드·글쓰기·분석까지 애매하게
   걸립니다. 안 쓰는 경우(수집·인프라·SQL·CI, 보고서 글쓰기, 데이터 분석, 색·간격 미세 조정)를
   명시했습니다.
2. **데이터 경계를 박았습니다.** 원문에 없는 조항입니다. 상담 내용·카카오/잔디 대화·개인정보·
   사내 문서 본문·AMS 내부 화면은 올리지 않고, 평소 나가는 것은 검색어뿐입니다. 이미지를
   보내는 기능은 공개 화면이거나 사용자가 올리라고 한 경우에만 씁니다.
3. **분량을 줄였습니다.** CLAUDE.md는 21장 규칙상 목차 역할이라, 항상 로드되는 쪽에는
   발동 조건·제외 조건·데이터 경계만 남기고 스킬 선택표와 검색 이어가기 절차는
   `lazyweb` 스킬로 내렸습니다(발동할 때만 로드).

원문에서 그대로 가져온 것도 있습니다. `lazyweb-growth-report`를 사용자가 직접 요청할 때만
쓰는 규칙은 공급사 지침과 같습니다.

**끄는 법**: `CLAUDE.md`에서 `LAZYWEB:ROUTER:BEGIN`부터 `END`까지를 지우면 됩니다.
꺼도 스킬 8종과 MCP 도구 43종은 그대로 쓸 수 있습니다. 자동 발동만 없어집니다.
