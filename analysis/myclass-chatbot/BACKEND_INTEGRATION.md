...
> 챗봇의 모든 상담/조회 데이터는 **현재 선택된 지점·학생의 실제 AMS 회원 데이터**로 채운다. 프로토타입은 이 바인딩을 **더미(김시대·김보민)**로 시연하며, **실제 회원 데이터는 저장소에 하드코딩하지 않고 런타임에 조회**한다.

- **컨텍스트(`CTX`)**: `{ campus(지점), student(학생), grade(학년) }`. 모든 `GET /me/*`는 이 컨텍스트의 학생을 대상으로 한다.
  - **학생 앱**: 진입 시 앱의 현재 선택 지점이 기본값. 챗봇 안에서 **지점 변경** 가능(4개 지점: 대치·목동·반포·분당). 지점은 상담실 연결처·교재 위치 등 지점별 데이터에 반영.
  - **학부모 앱**: 진입 시 현재 지점의 자녀가 기본값. 챗봇 안에서 **지점·자녀(학생) 변경** 가능(여러 자녀). 자녀 변경 시 결제·출결·시간표가 그 자녀 데이터로 전환.
...
- **PII**: 이름·연락처 등은 §0 규칙대로 마스킹. **실제 회원의 개인정보를 프로토타입/저장소에 적재하지 않는다**(데모는 더미만).
