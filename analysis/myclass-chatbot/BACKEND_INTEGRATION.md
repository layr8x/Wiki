...
- **PII**: 이름·연락처 등은 §0 규칙대로. **실제 회원의 개인정보를 프로토타입/저장소에 적재하지 않는다**(데모는 더미만).
...
- **단일 출처 배열 유지**: SSOT의 `PAY_DUE`/`PAY_PENDING`처럼 한 납부 응답을 `pay_wait`가 섹션별로 재사용한다. 실 API도 응답 하나를 노드가 공유해 **항상 일치**시킨다. 빈 상태(완납·내역 없음·강좌 없음)의 문구는 `기획서_v6_요약.md §3` 정본을 따른다.
...
| 단계 | 범위 |
|---|---|
| 1차 | 읽기 전용 조회(출결·보강·납부·결제·시간표) — 위험 낮고 효용 큼. 매핑 = §1 |
