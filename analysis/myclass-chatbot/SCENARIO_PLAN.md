...
8개 메뉴 · 약 80개 대화 상태 · 3개 연결 채널로 구성.
...
flowchart LR
  Q{"문제 성격은?"}:::step
  Q -->|"학사·계정·결제·환불·일반"| C1["☎ 지점 상담 실장<br/>전화 02-552-2373<br/>문자 010-5423-2378"]:::hand
...
| 채널 | 담당 | 연결되는 문의 |
|---|---|---|
| **지점 상담 실장** (`handoff`) | 대치 캠퍼스 고3 상담실 | 통합회원·자녀계정, 미납·환불·오입금, 결제수단 변경, 반 변경/전반, 퇴원, 자습실, 컨설팅, 일반 상담 |
| **마이클래스 카톡** (`handoffTech`) | 기술 담당 | 로그인 실패, 앱 오류(흰화면·튕김·설치), 영상 재생 오류, 특강 본인인증 미수신 |
| **시대인재 LIVE 카톡** (`handoffLive`) | 라이브 담당 | 라이브 입반·교재 반납·환불·현강 이동 등 라이브 서비스 전반 |

> 야간(22~08시)에는 핸드오프 시 "문자로 남겨두면 운영 시작 시 먼저 연락" 안내가 자동 추가됨.
...
flowchart TD
  ...
  SC --> CERT["납입증명서<br/>기간 × 항목 선택"]:::step
  SC --> SMS
  SMS --> SMS2["미납 N건 → 건별/전체 링크 발송"]:::solve
  F --> F1["재시도 / 문자 재발송"]:::step
  F1 -.안 되면.-> CP["☎ 지점 상담 실장"]:::hand
  R --> CP
  E --> E1["현금영수증 / 카드변경 / 오입금"]:::step
  E1 -.카드변경·오입금.-> CP
...
flowchart TD
  ...
  TR --> TR1["강좌 선택 → 같은 강사 다른 시간"]:::step --> CP["☎ 지점 상담 실장"]:::hand
...
flowchart TD
  D["④ 라이브·교재·배송"]:::menu
  D --> D1["라이브 입반 / 교재 반납 / 결제 / 환불 / 현강 이동"]:::step
  D --> D2["복습·추가영상"]:::step
  D --> D3["교재 배송 조회"]:::step

  D1 --> CL["💬 시대인재 LIVE 카톡<br/>(학생·강좌·요청내용)"]:::hand
  D2 --> D2a["앱 → 수강 강좌 → 강좌 상세 → 강의 목록"]:::step
  D2a -.영상 안 열림.-> VID["영상 오류 진단"]:::step
...
4. **맥락에 맞는 채널** — 학사=상담실장 / 앱·영상=마이클래스 카톡 / 라이브=LIVE 카톡.
