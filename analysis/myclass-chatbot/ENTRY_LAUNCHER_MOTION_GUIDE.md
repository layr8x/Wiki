...
>
> **결론**: 5차까지 쌓아온 글로우(`::before`/`::after`)·스펙큘러 스윕(`.sheen`)·계속 도는 3D 틸트·스프링 이징을 **전부 제거**. 대신 ①대기 = 눈 깜빡임(변경 없음) + 아주 미세한 `rotate` 갸웃 ②호버 = 절제된 `scale(1.05)`만 ③**화려한 3D 회전은 클릭(챗봇 열기) 순간에만 "동전 뒤집기"(`rotateY 0→360deg`)로 1회** 사용 — 로 재구성했다. 아이콘 도형(bub·eye·smile)과 그 좌표는 이번에도 손대지 않음.
...
| **클릭 = "동전 뒤집기"(신규)** | `svg.flip` | `oflip`: `rotateY(0→360deg)` — Face ID 스캔→완료 전환 참고, 계속 도는 장식이 아니라 클릭 시 1회 | .5s **1회** (JS가 클래스 토글로 재생) | ease-in-out |
| 프레스 | 컨테이너 | `scale(.8333)` | .1s | ease-out |

**클릭 플립 구현 메모**: 예전 `.ripple`(별도 DOM 요소로 만드는 링)을 완전히 대체 — 이제 클릭 피드백은 아이콘 자체(`svg`)가 `flip` 클래스를 받아 도는 것 하나뿐이다. JS `onclick`에서 `classList.remove('flip')` → `void offsetWidth`(강제 리플로우) → `classList.add('flip')` 순서로 **매번 재생 가능**하게 처리.

> ⚠️ **버그 발견·수정**: 처음 구현했을 때 호버 상태에서 클릭하면(=실제 사용자의 일반적인 클릭 경로) 플립이 전혀 안 보이는 버그가 있었다. 원인은 CSS 스펙상 **`transition`이 같은 속성(`transform`)을 노리는 `animation`보다 우선순위가 높다**는 점 — `#openBtn:hover svg{transform:scale(1.05);transition:transform .32s var(--ease)}`가 호버 중 계속 활성 상태라, `.flip`의 `animation:oflip`이 같은 `transform` 속성을 바꾸려 해도 transition이 이겨서 화면엔 아무 변화가 없었다(콘솔에서 `getComputedStyle`로 확인하면 애니메이션 자체는 정상 작동 중이었음 — 눈에만 안 보였던 것). **해결**: 호버 규칙의 셀렉터를 `#openBtn:hover svg:not(.flip)`으로 좁혀서, `.flip`이 붙는 순간 호버의 정적 transform·transition이 아예 매칭되지 않게 함. Playwright로 idle→hover→클릭 전체 시퀀스를 캡처해 프레임별로 아이콘 폭이 점점 줄었다가(옆모습) 다시 늘어나는(정면) 걸 직접 확인해서 검증.

**색**: 완전 무채색 — `--icon-primary:#161616`(진한 회흑색) 하나만 사용. 5차까지 있었던 `--glow-tint`·글로우·스펙큘러용 파란 색상 전부 제거.

**검증**: Playwright로 idle 5s(고개 갸웃 한 주기)·hover 전환+유지·클릭 플립까지 총 42프레임 캡처 → 애니메이션 GIF로 합성해 육안 확인. 플립 프레임을 4열 그리드로 늘어놓고 아이콘 폭이 연속적으로 줄었다 늘어나는 걸 확인해 "동전 뒤집기"가 실제로 작동함을 검증(§15와 같은 원칙 — 검증 전 "완료" 금지). 빌드·린트·테스트(206개) 통과.
