...
> **한 줄 정의**: 마이클래스 앱(시대인재) 안에서 학생·학부모가 **버튼만으로 스스로 문제를 해결**하고, 안 되면 **성격에 맞는 정확한 창구로 연결**하는 흑백 미니멀 챗봇 프로토타입. (챗봇 명칭 **"마이클래스 도우미"** · 서브 "문제를 바로 해결하도록 도와드려요")
>
...
| **시나리오** | 8개 메뉴 · 약 81개 대화 상태(막다른 길 0). 모든 가이드는 실제 상담 데이터(카카오 7,213대화·AMS 424,200행)와 **실제 앱 화면**으로 검증. |
...
| **검증** | QA 크롤러(약 81개 상태·오류 0·막다른 길 0)·헤드리스 렌더(3뷰)·실제 계정 로그인 검증·운영 배포 확인. |
...
| # | 메뉴(타일) | 아이콘 | 주요 노드 |
|---|---|---|---|
| 1 | **계정·로그인·앱** | key | `ac_login`(통합회원 연결·아이디찾기·비번재설정·카톡 기술상담) · `ac_unify`(통합회원·자녀계정) · `ac_app`(흰화면·튕김·업데이트 후·설치) |
| 2 | **미납·결제·환불** | credit_card | `pay_status`·`pay_sms`·`pay_va`·`pay_cert`(+`cert_period`·`cert_item`)·`pay_fail`·`pay_refund`·`pay_etc`(현금영수증·결제수단변경·오입금) |
| 3 | **입반·등록·대기** | how_to_reg | `en_reg`(영역→강좌)·`en_check`·`en_wait`·`en_waitinfo`·`en_waitset`·`en_special`(특강 본인인증)·`en_transfer` |
| 4 | **라이브·교재·배송** | live_tv | `m_live`(입반·복습영상·배송·반납·결제·환불·라→현 이동) · `live_video`(영상 4갈래) |
| 5 | **출결·보강** | fact_check | `at_makeup`(동영상 보강) · 오늘출결 · `at_week`(주간 히트맵) · 현장보강 · 결석 사전신고 |
| 6 | **수업·시간표** | calendar_month | `time_table`(내 시간표·이번주/다음주) · 강의실·서바이벌 커리큘럼·종강/휴강 |
| 7 | **퇴원·전반** | logout | `m_quit`(시간·난이도·강사·라이브·계획) · `quit_refund`(2단계 확인) |
| 8 | **상담원·자주묻는것** | support_agent | `help_faq` · `help_contact` · 교재수령(내 강좌 전체)·연구소교재비·설명회·자습실(부엉이 라이브러리) (사람 연결은 맨 끝에 배치) |

> 메뉴 타일 배열 순서(MENU): ① account → ② pay → ③ enroll → ④ live → ⑤ attend → ⑥ time → ⑦ **quit(퇴원·전반)** → ⑧ **help(상담원·자주묻는것)**.

주요 노드(NODES 키): `home, m_account, ac_login, ac_unify, ac_app, m_pay, pay_etc, pay_status, pay_sms, pay_va, pay_cert, cert_period, cert_item, pay_fail, pay_refund, m_enroll, en_reg, en_check, en_wait, en_waitinfo, en_waitset, en_special, en_transfer, m_live, live_video, m_attend, at_week, at_makeup, time_table, m_time, m_help, help_contact, help_faq, m_quit, quit_refund`. 여기에 칩 콜백(`act:`)으로 갈라지는 세부 분기와 헬퍼(`handoff`/`handoffTech`/`handoffLive`/`regPick`/`waitPick`/`makeupWeeks`/`certIssue` 등)가 만드는 상태까지 합쳐 **약 81개 대화 상태**로 구성, **막다른 길 0**.
...
| 채널 | 함수 | 연결 대상 | 넘기는 케이스 |
|---|---|---|---|
| **학원 상담실 전화** | `handoff()` | 대치 캠퍼스 고3 상담실<br>전화 **02-552-2373** · 문자 **010-5423-2378** | 환불(회차 기준)·통합회원/자녀계정·결제수단(카드) 변경·오입금 정정·납입증명서 발급(상담 선생님이 엑셀 .xlsx)·전반(시간/난이도/강사)·연구소 교재비·현장보강·서바이벌 커리큘럼·종강/휴강·컨설팅·자습실·퇴원 — **학사·정산** |
| **시대인재 마이클래스 카톡** | `handoffTech()` | `pf.kakao.com/_VGAQn/chat` | 로그인 실패·앱오류(흰화면·튕김·업데이트 후·설치)·VOD영상 재생 오류·특강 본인인증 문자 미수신 — **앱/계정 기술** |
| **시대인재 LIVE 카톡** | `handoffLive()` | `pf.kakao.com/_TkpPG/chat` | 라이브 입반·라이브 환불·라→현강 이동·라이브 교재 반납 — **별도 LIVE 일반 상담**(기술지원 아님) |

> 사용자 응대 호칭은 **"상담 선생님"**(과거 '실장' 표현 폐기). 챗봇이 사람으로 연결할 때 "상담 선생님이 확인해 드려요" 식으로 안내.
...
| 기능 | 데이터 근거 |
|---|---|
| 🔑 1순위 배치 + 앱오류 세분화(흰화면·튕김·업데이트 후·설치) | 카카오 1위 36.7%, 앱오류 키워드 1,486건(로그인 1,269보다 많음) |
| 로그인=**시대인재 통합회원(이메일 아이디+비밀번호)** 안내 + 아이디찾기·비밀번호 재설정 | 통합회원 단일 로그인 체계(소셜 로그인 아님 — 미검증 소셜 가입 안내는 삭제) |
...
| 홈 '자주 찾는 것' FAQ 노출 | "기타 FAQ" 28.2%가 맨 끝 타일(⑧ 상담원·자주묻는것)에 묻혀 누수 → 홈에 직접 노출(환불·보강·앱) |
...
| 방법 | 내용 | 최종 결과 |
|---|---|---|
| **QA 크롤러**(jsdom) | 전 노드 BFS 클릭 — JS오류·막다른길·미마스킹 PII·라벨없는 버튼 | 약 81개 상태 · 오류 0 · 막다른길 0 · 라벨없는 버튼 0 (노출 번호 `010-5423-2378`은 상담실 공개번호, 가상계좌 `110-204-567890`은 전용계좌 전체노출+복사) |
...
- **카카오 채널 딥링크 검증** — 실제 앱 내 카카오 상담 채널 진입 경로와 채널 챗 링크(`_VGAQn`·`_TkpPG`)의 일관성. (구체적 앱 메뉴 경로는 미검증이라 단정하지 않음.)
