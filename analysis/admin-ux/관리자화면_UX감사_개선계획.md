...
**1차 8건 + 2차 10건, 17개 작업 전부 반영 완료.** 남은 것은 4장의 보류 8건(김명준님 판단 대기)뿐이다.

2차에서 한 것 중 눈에 잘 안 띄지만 중요한 것:

| 항목 | 무엇이 잘못돼 있었나 | 확인한 숫자 |
|---|---|---|
| 2-1 클릭 가능화 | 화면에서 유일하게 "지금 뭘 해야 하는지" 말하는 목록인데 그리로 갈 방법이 없었다 | RPC 에 chat_id·profile_id 추가, 마우스·키보드(Enter) 둘 다 동작 확인 |
| 2-2 다크 표면 | 다크에서 body·card·muted 가 전부 #1b1b1b 라 카드 위 면이 사라졌다 | 대비 1.00 -> 1.14(라이트 1.13 과 같은 정도), 스켈레톤 1.00 -> 2.20 |
| 2-3 3상태 분리 | 5개 화면 중 3개가 조회 실패와 빈 결과를 픽셀 단위로 똑같이 보여줬다 | 5개 화면 전부 오류·빈 화면 md5 가 달라짐 |
| 2-4 필터 접근성 | 네 화면의 필터에 선택 상태를 알리는 속성이 하나도 없었다 | role="radio" + aria-checked, 툴바 높이 20/28/32 -> 28 통일 |
| 2-5 잔디 적층 | 1440x900 첫 화면에 대화가 0줄이었다 | 목록 시작 965 -> 873px, 1920px 에서 927 -> 738px |
| 2-6 차트 접근성 | 막대 30개에 눈금 0개, 값은 브라우저 기본 말풍선뿐 | role="img" + 요약 aria-label + 날짜 눈금 3개 + 부정 비율 상시 표기 |
| 2-7 대시보드 링크 | 본문 링크가 전부 /editor 였고 상담 화면 링크는 0개 | 카드 3장에 "상담 로그에서 보기", KPI 768px 에서 3+1 -> 4열 |
| 2-8 피드백 표 | 390px 에서 표의 43%가 화면 밖 | 좁은 폭 5열 -> 3열, 전문은 Dialog |
| 2-9 가이드 표 | 상태 배지가 '임시저'로 잘려 "깨졌다"로 읽혔다 | 1100px 미만 6열 -> 3열, 액션 168 -> 56px |
| 2-10 공용 chrome | 화면을 옮기면 본문 폭이 튀었다 | 다섯 화면 본문 폭·시작 위치 완전 일치(1024/1440/1920px 실측) |

**2차에서도 대역 데이터 결함이 5건 더 나왔다**(누적 10건). 전부 "필터를 실제로 동작시키자마자"
또는 "그 값을 화면에 쓰자마자" 드러난 것들이다.

| 드러난 것 | 원인 |
|---|---|
| 채널을 바꿔도 목록이 그대로였다 | 대역의 모든 필터가 no-op 이었다 — 필터 동작을 이 도구로 한 번도 확인할 수 없었다 |
| 잔디 목록이 통째로 비었다 | 대역 `room_id` 가 'room0' 같은 임의 값이라 실제 방 id 와 달랐다 |
| 피드백 유형 배지가 전부 "기타", 탭 건수 전부 0 | 대역 컬럼이 `rating` 인데 실제 조회는 `vote` 를 읽는다 |
| 가이드 타입 배지가 빈 알약, TL;DR 줄이 안 보임 | 대역에 `type`·`tldr` 이 없었다 |
| 감정 차트 가로축이 뒤로 갔다 앞으로 왔다 | 대역 날짜가 `15 + (i % 17)` 이라 같은 구간을 반복했다 |

**결론(15장에 추가할 것)**: 화면 정합 실패의 뿌리는 "낡은 노드와 비교" 하나가 아니다.
**틀린 대역 데이터와 비교하는 것**이 같은 크기의 두 번째 뿌리였고, 이번에 10건이 나왔다.
새 화면을 검증하기 전에 **대역이 실제 응답과 같은 컬럼 이름·같은 형식인지부터** 확인할 것.

1차를 하면서 **계획서에 없던 결함 5건**이 추가로 드러났고 함께 고쳤다. 전부 검증 대역(fixtures)
자체가 틀려 있어서 그동안 감사에 안 잡히던 것들이다.

| 추가로 드러난 것 | 어떻게 드러났나 |
|---|---|
| 분포 카드가 "NaN%" 를 찍고 막대가 전부 꽉 차 있었다 | 대역에 `pct` 가 없었다. 버킷 이름도 실제 RPC 형식이 아니라 색 매핑에 하나도 안 걸렸다 |
| 잔디 메시지가 전부 "(본문 없음)" 으로 렌더 | 대역의 본문 컬럼 이름이 `message` 가 아니라 `text` 였다. 이 화면의 글자 길이·줄바꿈은 한 번도 측정된 적이 없었다 |
| 모듈별 분포가 전 항목 0 | 대역의 `module` 값이 `MODULE_TREE` 라벨과 달랐다 |
| 잔디 스레드 제목이 390px 에서 91% 가려짐 | 위 대역을 고치자마자 드러났다 |
| 잔디 채널 카드 방 이름이 1920px 을 뺀 모든 폭에서 잘림 | 같은 이유 |

또 `render.cjs` 에 `--height` 를 넣었다. 관리자 껍데기가 본문에 자체 스크롤 상자를 써서
`fullPage` 스크린샷이 창 높이(1000px)에서 잘렸고, 아래쪽 카드를 아무도 못 보고 있었다.

**교훈(15장과 같은 뿌리)**: 낡은 노드와 비교하는 것만 문제가 아니라, **틀린 대역 데이터와 비교하는
것도 같은 실패**다. 화면을 고치기 전에 대역이 실제 응답과 같은 모양인지부터 확인할 것.
...
**2026-08-13 실제 처리 결과 (일부만 반영)**

- 반영: 잔디를 상담과 같은 2열 구조로 바꿨다. 대화 목록 시작 위치 **965px -> 873px**(1440px), **927px -> 738px**(1920px). 1440x900 첫 화면에 대화가 0줄이던 것이 1줄, 1920에서는 2줄이 됐다. KPI 그리드 minWidth 200 -> 160 으로 한 줄 유지.
- **반영 안 함(의도적)**: 채널 KPI 카드 제거·`SelectableCard` 전환.
  - 카드 제거는 김명준님이 직접 "채널별 카드 한 줄로"라고 지시한 것을 되돌리는 일이다.
  - `SelectableCard` 전환은 방금 접근성 때문에 넣은 `SegmentedControl`(선택 상태를 읽어주는 유일한 장치)을 없애는 일이라 손해가 더 크다.
- 남은 사실: 상담 화면은 평소(경보 없음) 상태에서도 목록 시작이 **1,345px**다(실측). 실시간 위젯 자체가 879px 이고 오른쪽 열은 411px 라 오른쪽에 468px 빈 공간이 남는다. 이건 위젯 안 정보량을 줄이는 판단이 필요해 김명준님 결정 대기로 넘긴다(4장).
...
| 잔디 스레드 원글 중복 | 헤더 제목과 첫 행이 같은 원글이다. ~~픽스처 산물~~ **(2026-08-13 픽스처 수정 완료 — `message`·`reply_to_message_id`·`content_type` 채움. 이제 실제 글자로 렌더된다.)** 실데이터는 코드 주석에 "댓글이 전체의 70%"라고 기록돼 있다 | 헤더 제목 유지 + `messages.slice(1)` 방향이 안전. 진행 여부만 결정하면 된다 |
| `Skip to content` 영어 문구 | AppShell이 자동 삽입하고 문구를 바꿀 prop을 문서에 노출하지 않는다 | Astryx 팀에 옵션 요청 |
| 햄버거 버튼 읽어주는 이름 `Open navigation` | 같은 뿌리. **1-6에서 시도했다가 되돌렸다** — `hasToggle:false` + `<MobileNavToggle label="메뉴 열기" />` 직접 배치는 좁은 폭에서 햄버거가 통째로 사라진다(AppShell이 자체 "모바일 바"를 그리며 TopNav의 `startContent`를 버린다, 실측). `MobileNavConfig`에 라벨 값이 없다 | 위 `Skip to content`와 함께 Astryx 팀에 요청 |
